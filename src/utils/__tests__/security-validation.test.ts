...
describe('Security Validation', () => {
  describe('validateUrl', () => {
    ...
    it('should reject URLs from unauthorized domains', () => {
      ...
      expect(() => validateUrl('https://evil.com/malicious')).toThrow("URL domain 'evil.com' is not in the allowed list");
      expect(() => validateUrl('https://api.figma.evil.com/files/abc')).toThrow("is not in the allowed list");
      expect(() => validateUrl('https://not-figma.com/files/abc')).toThrow("is not in the allowed list");
  ...
  describe('validateHeaders', () => {
    ...
    it('should reject headers with dangerous characters in keys', () => {
      ...
      dangerousKeys.forEach(headers => {
        expect(() => validateHeaders(headers as unknown as Record<string, string>)).toThrow('Header key contains potentially dangerous characters');
      });
    });
    ...
    it('should reject headers with dangerous characters in values', () => {
      ...
      dangerousValues.forEach(headers => {
        expect(() => validateHeaders(headers as unknown as Record<string, string>)).toThrow('Header value contains potentially dangerous characters');
      });
    });

    it('should reject headers with injection patterns', () => {
      // Command substitution patterns should be caught
      expect(() => validateHeaders({ 'Authorization': 'Bearer token$(whoami)' })).toThrow('Header value contains potentially dangerous characters');
      expect(() => validateHeaders({ 'Authorization': 'Bearer token${USER}' })).toThrow('Header value contains potentially dangerous characters');
      // Variable assignment patterns should be caught by injection pattern check
      expect(() => validateHeaders({ 'Custom-Header': 'value; test=dangerous' })).toThrow('Header value contains potential injection pattern');
    });
    ...
    it('should reject headers that are too long', () => {
      ...
      expect(() => validateHeaders({ [longKey]: 'value' })).toThrow('Header key exceeds maximum length');
      expect(() => validateHeaders({ 'key': longValue })).toThrow('Header value exceeds maximum length');
    });

    it('should reject invalid header inputs', () => {
      expect(() => validateHeaders({ '': 'value' })).toThrow('Header key must be a non-empty string');
      expect(() => validateHeaders({ 'key': '' })).toThrow('Header value must be a non-empty string');
      expect(() => validateHeaders({ 'key': null as any })).toThrow('Header value must be a non-empty string');
    });
  });
  ...
  describe('validateCurlCommand', () => {
    ...
    it('should reject commands with dangerous patterns', () => {
      ...
      dangerousCommands.forEach(command => {
        expect(() => validateCurlCommand(command)).toThrow('Curl command contains potentially dangerous patterns');
      });
    });
